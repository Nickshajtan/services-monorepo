...
const main = () => {
    try {
        ...
        if (!baseFile || !prFile) {
            console.error(
                kleur.red('Usage: node check-async-breaking.js <base-events.yaml> <pr-events.yaml>')
            );
            process.exit(1);
        }
        ...
        const extractChannelOps = (spec) => {
            const channels =
                spec.channels && 'object' === typeof spec.channels ? spec.channels : {};
            const out = {};
        ...
        const extractPayloadRefAndSchema = (messageObj, resolver, specFilePath) => {
            ...
            if (Array.isArray(messageObj.oneOf)) {
                ...
                return {
                    payloadRef: null,
                    payloadSchema: { oneOf: variants.map((x) => x.payloadSchema).filter(Boolean) },
                    variants,
                };
            }
        ...
        const compareRequiredDeep = (baseSchema, prSchema, where, errors) => {
            ...
            const getProperties = (schema) => {
                ...
                return schema.properties && 'object' === typeof schema.properties
                    ? schema.properties
                    : {};
            };
            ...
            if (isObjectSchema(base) && !isObjectSchema(pr)) {
                errors.push(
                    `Breaking: schema at ${where} changed from object-like to non-object-like`
                );
                return;
            }
            ...
            if (Object.keys(bProps).length && Object.keys(pProps).length) {
                for (const required of bReq) {
                    if (!(required in pProps)) {
                        errors.push(
                            `Breaking: required field "${required}" no longer exists in properties at ${where}`
                        );
                    }
                }
            }
            ...
            for (const kw of ['oneOf', 'anyOf']) {
                if (Array.isArray(base[kw])) {
                    if (!Array.isArray(pr[kw]) || pr[kw].length < base[kw].length) {
                        errors.push(
                            `Breaking: ${kw} at ${where} changed (missing or fewer variants)`
                        );
                        continue;
                    }
            ...
            for (const opName of Object.keys(baseOps)) {
                ...
                if (basePayload.payloadRef) {
                    if (!prPayload.payloadRef) {
                        errors.push(
                            `Breaking: payload ref removed/changed at ${where} (was ref, now inline or missing)`
                        );
                    } else if (basePayload.payloadRef !== prPayload.payloadRef) {
                        errors.push(
                            `Breaking: payload $ref changed at ${where}\n  base: ${basePayload.payloadRef}\n  pr:   ${prPayload.payloadRef}`
                        );
                    }
                }
                ...
                if (basePayload.payloadSchema && prPayload.payloadSchema) {
                    compareRequiredDeep(
                        basePayload.payloadSchema,
                        prPayload.payloadSchema,
